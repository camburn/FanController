...
class RFDevice:
    ...
    def tx_code(self, code, tx_proto=None, tx_pulselength=None, tx_length=None):
        ...
        if tx_length:
            ...
            self.tx_length = 24
        rawcode = ("{0:0" + str(self.tx_length) + "b}").format(code)
        # rawcode = f"#0{self.tx_length + 2}b"
    ...
    def tx_bin(self, rawcode):
        ...
        print(f"TX Length: {self.tx_length}")
        print(f"TX Repeat: {self.tx_repeat}")
    ...
    def enable_rx(self):
        ...
        if not self.rx_enabled:
            ...
            self.pin = machine.Pin(self.pin_number, machine.Pin.IN)
            self.pin.irq(
                trigger=Pin.IRQ_RISING | Pin.IRQ_FALLING, handler=self.rx_callback
            )
